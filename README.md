...
适用于 90% 的场景,脚本会依次完成: **环境检查 → 编译 → 安装 → 加载 → 自检**。

```bash
# 1. 克隆仓库
git clone <your-fork-url> yt6801-linux-driver
...
> `install` 步骤中的 `make install` 与 `insmod` 需要 root 权限。脚本会自动用 `sudo`,
> 你也可以直接 `sudo ./yt_nic_install.sh`。
